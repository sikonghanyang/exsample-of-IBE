;;;;;;下列函数成功运行的条件是 per<0.7,否则to-report没有报告任何东西
...
set gene1 py:runresult"np.random.binomial(1, 0.2,5)"
set gene2 py:runresult"np.random.binomial(1, 0.2,5)"
set n  random  (length gene1)
  if  per < 0.7 [
  show n
  show "gene1"
  show gene1
  ...
  set s1 (transition1 gene1)
  set s2 (transition2 gene1)
  ;set s1 first-half
  ;set s2 behind-half

  set s3 transition1 gene2
  set s4 transition2 gene2
    ;set s3 first-half
  ;set s4 behind-half
